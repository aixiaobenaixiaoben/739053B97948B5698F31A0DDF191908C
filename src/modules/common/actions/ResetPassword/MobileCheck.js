...
export const mobileCheckSend = (data: Syusrinf): ActionAsync => {
  return (dispatch, getState) => {
    ...
    Request.post(URL_RESET_PASSWORD_MOBILE_CHECK_SEND, qs.stringify(data))
      .then(response => {
        const {RTNCOD, ERRMSG} = response.data
        if (RTNCOD === 'SUC') {
          dispatch({type: ACTION_RESET_PASSWORD_MOBILE_CHECK_SEND, payload: data})
          Toast.hide()

          let count = DURATION_MOBILE_CODE_EXPIRED
          let timer = setInterval(() => {
            dispatch({type: ACTION_RESET_PASSWORD_MOBILE_CHECK_COUNT})
            count = count - 1
            if (count === 0) {
              clearInterval(timer)
            }
...
export const mobileCheck = (data: Syvrymbl): ActionAsync => {
  return (dispatch, getState) => {
    ...
    Request.post(URL_RESET_PASSWORD_MOBILE_CHECK, qs.stringify(data))
      .then(response => {
        const {RTNCOD, RTNDTA, ERRMSG} = response.data
        if (RTNCOD === 'SUC') {
          dispatch({type: ACTION_RESET_PASSWORD_MOBILE_CHECK_SUC, payload: RTNDTA})
          Toast.hide()
        } else {
          Modal.alert('', ERRMSG)
        }
